...
- [x] `decoy compile pipeline.yaml --explain` — per-column strategy/params/rationale shown. **Verified** against the `decoy init` STORM-scaffolded `pipeline.yaml`: clean per-column table (strategy/provider/cardinality/vault/rationale), correct "no FK dependencies detected" ordering note (single-table config), and an accurate `23 passed, 3 skipped (no-profile mode)` compile-check breakdown — the 3 skips (`basic_uniqueness_pre_flight`, `pool_capacity_pre_flight`, `null_bearing_int_unsupported`) are exactly the profile-dependent checks, consistent with the `decoy plan --no-profile` finding above. Good honesty disclaimer at the bottom: "explains declarations in the config. It does not guarantee the strategy is correct for the data... Use `decoy storm analyze` to inspect source data first."
